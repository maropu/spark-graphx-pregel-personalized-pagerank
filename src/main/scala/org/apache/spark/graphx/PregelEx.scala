 ...
 *
 */
object PregelEx extends Logging {
  ...
  def apply[VD: ClassTag, ED: ClassTag, A: ClassTag]
     (graph: Graph[VD, ED],
      ...
      funcToComputeGlobalMetric: Option[((VD, VD) => VD, VD => String)] = None)
    ...
    : Graph[VD, ED] =
  {
    ...
    while (activeMessages > 0 && i < maxIterations) {
      ...
      funcToComputeGlobalMetric.foreach { case (reduceFunc, formatFunc) =>
        val metric = g.vertices.map(_._2).reduce { case (v1, v2) => reduceFunc(v1, v2) }
        logWarning(
          s"""
             |Pregel finished iteration $i (# of active messages: $activeMessages)
             |Aggregated metric: ${formatFunc(metric)}
           """.stripMargin)
      }
